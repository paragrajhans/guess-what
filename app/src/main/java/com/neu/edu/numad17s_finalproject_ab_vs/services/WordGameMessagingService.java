...
public class WordGameMessagingService extends FirebaseMessagingService {
    ...
    private void sendNotification(RemoteMessage.Notification notification) {
        ...
        intent.putExtra(Constants.BODY,notification.getBody());
        intent.putExtra(Constants.CLICK_ACTION,notification.getClickAction());

        // check if the game is in progress and new state is received
        if (intent.getStringExtra(Constants.CLICK_ACTION).equals(Constants.CHALLENGE_ACCEPTED)) {

            Utils.saveChallengeKey(intent.getStringExtra(Constants.BODY), getApplicationContext());
//            MainActivity.instance.transact(LiveGameBoardFragment.newInstance(true));
            Router.getInstance().showShowAndGuessFragment();

        }
        // shows the notification when user makes the move in Async Game
        else if(intent.getStringExtra(Constants.CLICK_ACTION).equals(Constants.ASYNC_MOVE_MADE)){
